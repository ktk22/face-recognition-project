# -*- coding: utf-8 -*-
"""face recognition cccd

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Gx4e7XfdSsTfbULmFRtk-qvbsRpvOoVj
"""
...
def calculate_cosine_similarity(face1, face2):
    ...
    face1_vector = face1_resized.flatten().reshape(1, -1)
    face2_vector = face2_resized.flatten().reshape(1, -1)

    # Tính cosine similarity
    similarity = cosine_similarity(face1_vector, face2_vector)[0][0]
        ...
        if cccd_faces is not None:
            for face in cccd_faces:
                ...
                if euclidean_distance is not None:
                    ...
                    cosine_sim = calculate_cosine_similarity(face_image_np, face_image_cccd_np)
                    print("Độ tương đồng cosine similarity giữa hai khuôn mặt:", cosine_sim)
                    # Đánh giá độ chính xác
                    threshold_cosine = 0.9  # Ngưỡng
                    if cosine_sim > threshold_cosine:
                        print("Khuôn mặt từ ảnh cắt khớp với khuôn mặt từ ảnh với độ tương đồng là {:.2f}%.".format(cosine_sim * 100))
                else:
                    print("Khuôn mặt từ ảnh cắt không khớp với khuôn mặt từ ảnh.")
    else:
        print("Không tìm thấy khuôn mặt từ camera.")
